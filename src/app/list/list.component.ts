...
export class ListComponent implements AfterViewInit{
  ...
  //複製貼上Angular表單
  displayedColumns: string[] = ['id', 'name', 'description', 'start_date','end_date','published'];
  @ViewChild(MatPaginator) paginator!: MatPaginator;

  QuestionnaireData: QuestionnaireFormat[] = [];
  // dataSource = new MatTableDataSource<QuestionnaireFormat>([]);
  ...
  // 載入問卷
  ngOnInit(): void {
    this.http.getApi<QuestionnaireFormat[]>('http://localhost:8080/quiz').subscribe({
      next: (res) => {
        console.log('取得的問卷資料:', res); // 確認資料是否正確
        this.QuestionnaireData = res;
      },
      error: (err) => {
        console.error('取得問卷資料時發生錯誤:', err); // 錯誤處理
      },
    });
  }
  dataSource = new MatTableDataSource<QuestionnaireFormat>(this.QuestionnaireData);
  ...
  ngAfterViewInit() {
    this.dataSource.paginator = this.paginator; // 確保分頁器與表格綁定
  }
  //======================================================================================
...
storeData(name: string,start_date: string,end_date:string,dcp: string){
  this.service.questionName = name;
  this.service.questionStartTime = start_date;
  this.service.questionEndTime = end_date;
